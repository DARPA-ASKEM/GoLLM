...
# PETRINET_TEMPLATE = escape_curly_braces(open('/Users/jwhiting/gollm/core/agents/petrinet.json', 'r').read()) # TODO: clean this up after demo.. fetch/cache/load .. etc
# PETRINET_PROMPT = """
...
def _model_config_chain(research_paper: str):
    print('Reading model config from research paper: {}'.format(research_paper[:100]))
    prompt = PETRINET_PROMPT.format(petrinet=escape_curly_braces(PETRINET_TEMPLATE), research_paper=escape_curly_braces(research_paper))
    client = OpenAI()
