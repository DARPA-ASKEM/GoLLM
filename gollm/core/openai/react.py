...
from core.openai.prompts.react import (SYSTEM_MESSAGE_PREFIX,
                                FORMAT_INSTRUCTIONS,
                                SYSTEM_MESSAGE_SUFFIX,
                                HUMAN_MESSAGE,
                                ACT_OBS)
from core.entities import Toolset, Message
...
class OpenAIAgent:
    ...
    def _construct_prompt(self, human_message: str, scratchpad: str = None):
        ...
        system_prompt += FORMAT_INSTRUCTIONS.format(tool_names=self.toolset.get_tool_code()) + '\n\n'
    ...
    def run(self, human_message: str, scratchpad: str = None):
        sys_prompt, human_prompt = self._construct_prompt(human_message, scratchpad=scratchpad)
        ...
        output = Message(message_content=output.choices[0].message.content, message_type='assistant')
...
class ScratchpadParser:
    ...
    def get_final_answer(cls, scratchpad: str):
        ...
        if match:
            ...
            raise ValueError(f'No final answer found in scratchpad.\nSCRATCHPAD:\n{scratchpad}')
    ...
    def get_observations(cls, scratchpad: str):
        return cls._strip_newlines(re.findall(cls.observation_pattern, scratchpad, flags=re.DOTALL))
    
    @classmethod
    def get_thoughts(cls, scratchpad: str):
        return cls._strip_newlines(re.findall(cls.thought_pattern, scratchpad, flags=re.DOTALL))
...
class ReActManager:
    ...
    def run(self, query: str):
        while True:
            ...
            actions = ScratchpadParser.get_actions(agent_output)  # TODO: ensure that all actions are indeed JSON blobs.

            if actions is None:
                self._increment_error(f"Error encountered when parsing agent output, no actions found: {agent_output}")

            # Assumption: We should not execute the exact same action twice within the react loop.
            pending_actions = [action for action in actions if action not in self.executed_actions]
            ...
            try:
                ...
                self._increment_error(f'No pending actions found in agent output: {agent_output}')
                continue

            ### log extraneous generated text ###
            print(f"Extraneous Text Truncated: {self.scratchpad[first_pending_action_index + len(pending_actions[0]):]}")

            self.scratchpad = self.scratchpad[:first_pending_action_index + len(pending_actions[0])]

            try:
                action_blob = json.loads(pending_actions[0])
            except json.decoder.JSONDecodeError:
                self._increment_error(f'Invalid json blob in agent output: {pending_actions[0]}')
                self.scratchpad.replace(pending_actions[0], '')
                continue

            executor_blob = self.executor.execute(json.loads(pending_actions[0]))
