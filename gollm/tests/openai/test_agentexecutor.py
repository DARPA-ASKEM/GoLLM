...
class TestToolsetAndAgentExecutor(unittest.TestCase):
    ...
    def setUp(self):
        self.sample_tool = Tool(name='sample_tool', args=['x'], description='Doubles the input', func=sample_tool_func, input_type=int)
        self.toolset = Toolset(tools=[self.sample_tool])
        self.agent_executor = AgentExecutor(toolset=self.toolset)

    def test_add_tool(self):
        new_tool = Tool(name='new_tool', args=['y'], description='New tool', func=lambda y: y + 1, input_type=int)
    ...
    def test_run_tool(self):
        ...
        self.assertEqual(result, {'tool_name': 'sample_tool', 'tool_output': 20, 'tool_input': 10})
