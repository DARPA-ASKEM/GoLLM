...
DATASET_PROMPT = """You are a helpful agent that will accept a representation of a mathematical model and one or more
dataset which contain parameter and initials values which must be mapped into the mathematical model. The
mathematical models may be represented in petri nets, reg nets, stock and flow, or other model formats. The datasets
may be in the form of a matrix where the index and column names are useful for mapping the values of initials and
parameters within the dataset to the model. Your goal is to map the dataset to the model and provide the results of
the mapping. If you cannot map the dataset to the model, use the string "null" as a placeholder. Use the following
model configuration as a reference:
...
configuration as your final answer.
Initials and Parameters should follow the format:\n
{
